...
export const kroegApiGetUncuratedTool = createTool({
  ...
  execute: async (inputData) => {
    const baseUrl = inputData.baseUrl || defaultBaseUrl;
    const params = new URLSearchParams();
    if (inputData.limit) {
      params.set("limit", String(inputData.limit));
    }
    if (inputData.category) {
      params.set("category", inputData.category);
    }

    const url = `${baseUrl}/api/curation/uncurated${params.size ? `?${params}` : ""}`;
    const apiKey = inputData.apiKey || process.env.KROEG_API_KEY;
...
export const kroegApiCreateDecisionTool = createTool({
  ...
  execute: async (inputData) => {
    ...
    return requestJson(url, {
      ...
      body: JSON.stringify({
        license_id: inputData.license_id,
        label: inputData.label,
        sanitized_name: inputData.sanitized_name,
...
export const kroegApiPendingTool = createTool({
  ...
  execute: async (inputData) => {
    const baseUrl = inputData.baseUrl || defaultBaseUrl;
    const authToken = inputData.authToken || process.env.KROEG_AUTH_TOKEN;
  ...
  execute: async (inputData) => {
    ...
    return requestJson(url, {
      ...
      body: JSON.stringify({
        ...
        override_label: inputData.override_label,
        override_name: inputData.override_name,
        override_reasoning: inputData.override_reasoning,
      }),
    });
  },
});
...
export const kroegApiSyncLicensesTool = createTool({
  ...
  execute: async (inputData) => {
    ...
    return requestJson(url, {
      ...
      body: JSON.stringify({ source_url: inputData.source_url }),
    });
  },
});
...
export const kroegApiSearchLicensesTool = createTool({
  ...
  description: "Search all licenses (curated and uncurated) by name, address, or feature id.",
  inputSchema: sharedAuthSchema.extend({
  ...
  execute: async (inputData) => {
    ...
    const params = new URLSearchParams();
    if (inputData.q) {
      params.set("q", inputData.q);
    }
    if (inputData.feature_id) {
      params.set("feature_id", inputData.feature_id);
    }
    if (inputData.limit) {
      params.set("limit", String(inputData.limit));
    }
