...
Many quadrants in the generation DB have incorrect color water - the aim of this new functionality is to correct this at scale.

An advanced AI agent (Gemini) gave me a target implementation of a "soft blend", where we replace a given target color with a given final color:

Here's what they said about it:

## Gemini Response

This is a great question that moves past basic image processing into concepts used in professional compositing.

The issue with the previous "Global Threshold" approach is that it creates a **hard, binary mask**. A pixel is either deemed "water" and completely replaced, or "not water" and left untouched.

In reality, edge pixels (anti-aliasing) are a mixture. A pixel on the edge of the truss might be 60% brown and 40% water. To look natural, your replacement color needs to respect that 40% opacity.

To achieve this, we need a **Soft Mask** (also known as an Alpha Matte). Instead of being just black or white, this mask will contain shades of gray indicating *how much* a pixel resembles the target color.

Here is the implementation using NumPy to calculate color distances and perform alpha blending.
...
1. **Floating Point Math:** We convert the image to floats (0.0 - 1.0). Standard image math happens on integers (0-255), which loses the precision necessary for subtle blending calculations.

2. **Color Distance (Euclidean Norm):** `np.linalg.norm(img_float - target_float, axis=2)` Instead of asking "Is this pixel in range? (Yes/No)", we ask: "Exactly how far away is this pixel's color from the water color in 3D RGB space?"
...
3. **The Soft Mask (Alpha):** We take that distance and invert it.
   ...
   - **Crucially**, if the distance is small (an edge pixel that is mostly water but slightly brown), the mask might be 0.8.

4. **Alpha Blending:** The final step uses the standard compositing formula: $Result = (NewColor \times Mask) + (OriginalPixel \times (1.0 - Mask))$ If the mask is 0.8 at an edge pixel, the final pixel becomes 80% bright magenta and 20% original brownish-water, perfectly preserving the anti-aliasing.

## Task

Your task is to implement a new script in the `src/isometric_nyc/e2e_generation` dir called "replace_color.py" that takes the following parameters:
...
Then, implement the code to load the png image pixels and update the pixels. Add a --dry-flag that just exports the updated png to the exports subdir of the generation dir without saving it back to the db.
